...
with tab6:
    st.header("🦅 Wire Hawk")
    st.markdown("""
    **Identify the best Free Agents instantly.** Since we don't sync with Yahoo/ESPN yet, simply **copy the list of available players** from your league page and paste them below.
    """)
    ...
    if not df.empty:
        ...
        with col_input:
            raw_text = st.text_area("Paste Player Names (one per line):", height=300, placeholder="Example:\nConnor McDavid\nAuston Matthews\n...")
        ...
        with col_result:
            if raw_text:
                ...
                if lines:
                    ...
                    # We use lowercase matching for better hit rates
                    df['match_name'] = df['Player'].str.lower()
                    search_names = [n.lower() for n in lines]
                    
                    found_players = final[final['Player'].str.lower().isin(search_names)].copy()
                    
                    if not found_players.empty:
                        st.subheader(f"✅ Found {len(found_players)} Players")
                        st.dataframe(
                            found_players[actual_cols].style.format("{:.2f}", subset=['Total Value'])
                                .background_gradient(cmap="RdYlGn", subset=['Total Value']),
                            width='stretch', height=600
                        )
                    else:
                        st.warning("No matches found. Check spelling or try standard NHL names.")
                else:
                    st.info("Paste names to start ranking.")
            else:
                st.info("👈 Paste your waiver wire list on the left to uncover hidden gems.")
    else:
